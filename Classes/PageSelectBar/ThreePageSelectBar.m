...
@implementation ThreePageSelectBar
...
- (instancetype)initWithFrame:(CGRect)frame options:(NSArray<NSString *> *)options selectBlock:(void (^)(NSString *, NSInteger))callBackBlock{
    ...
    _bottomBar = [[UIImageView alloc]initWithFrame:CGRectMake(kScreenWidth/6-20, self.frame.size.height - 3, 40, 2)];
...
- (void)bottomBarMoveToIndex:(NSInteger)index{
    [UIView animateWithDuration:0.2 animations:^{
        _bottomBar.frame = CGRectMake((self.frame.size.width/(_options.count)) * index+kScreenWidth/6-20, self.frame.size.height - 3, 40, 2);
    }];
}
