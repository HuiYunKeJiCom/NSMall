...
//@property (strong, nonatomic) NSMutableDictionary *modelCache;//模型字典
...
- (void)setupUI {
    ...
    if (![CLTools sharedTools].cornerRadius) {
        [CLTools sharedTools].cornerRadius = 0.2;
//        _tagHeight * 0.5;
    }
...
//    NSLog(@"%@", tagBtn.titleLabel.text);
    if (tagBtn.tagSelected) {
        ...
        [self addTagWithTag:tagBtn.model];
        _textFieldIsEditting = NO;
    }else {
        [self removeTagWithTag:tagBtn.model];
    }
    _selectedBtn.selected = NO;
}
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    SaveLabelParam *param = [SaveLabelParam new];
    param.type = @"1";
    param.labelName = textField.text;
    //调用标签生成接口,返回标签模型
    [GetLabelsAPI saveLabels:param success:^(LabelItemModel * _Nullable result) {
        DLog(@"新增标签成功");
//        [self.modelCache setObject:result forKey:result.label_name];
        [self addTagWithTag:result];
...
- (void)addTagWithTag:(LabelItemModel *)text {
    NSLog(@"label_name = %@",text.label_name);
    if (![self.tagCache objectForKey:text.label_name]) {
        CLTagButton *tagBtn = [[CLTagButton alloc] initWithTextField:_inputField];
        tagBtn.model = text;
        CGFloat width = [self tagWidthWithText:text.label_name];
        [tagBtn setTitle:text.label_name forState:UIControlStateNormal];
        ...
        [self.tagCache setObject:tagBtn forKey:text.label_name];
//        if (![self.modelCache objectForKey:text.label_name]) {
//            [self.modelCache setObject:text forKey:text.label_name];
//        }
...
- (void)removeTagWithTag:(LabelItemModel *)tag {
    CLTagButton *tagbtn = [self.tagCache objectForKey:tag.label_name];
    NSInteger index = [self.tagBtnArrayM indexOfObject:tagbtn];
    [self.tagCache removeObjectForKey:tag.label_name];
//    [self.modelCache removeObjectForKey:tag.label_name];
    ...
    if (tagTextField.text.length ==0) {
        if (self.tagBtnArrayM.count > 0) {
            ...
            if (!lastBtn.isSelected) {
                ...
                _selectedBtn.selected = NO;
                [self removeTagWithTag:lastBtn.model];
            }
        }
    }
}
...
- (NSArray *)tags {
    ...
    for (CLTagButton *btn in [self.tagCache allValues]) {
        [tagsArr addObject:btn.model];
    }
    return tagsArr;
}

- (void)setLabels:(NSArray<LabelItemModel *> *)labels {
    _labels = labels;
    
    for (LabelItemModel *des in labels) {
//        NSLog(@"des = %@",des.label_name);
//        [self.modelCache setObject:des forKey:des.label_name];
