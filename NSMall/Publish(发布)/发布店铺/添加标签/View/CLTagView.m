...
- (void)setupUI {
    ...
    headView.backgroundColor = [UIColor whiteColor];//cl_colorWithHex(0xf0eff3);
...
- (void)layoutTags:(NSArray<CLTagButton *> *)tags {
    ...
    if (self.displayTags.count > 0) {
        for (LabelItemModel *tagDes in self.displayTags) {
            CLTagButton *btn =[_tagsCache objectForKey:tagDes.label_name];
            if (btn) {
                btn.tagSelected = YES;
            }
        }
